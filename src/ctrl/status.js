...
function notify_progress() {
    ...
    document.getElementById('article_count_done_' + ctrl['mode']).innerText = count_done.toLocaleString()
    document.getElementById('article_count_total_' + ctrl['mode']).innerText = count_total.toLocaleString()
    ...
    if (done_rate > 0.1) {
        now = new Date()
        elapsed_sec = Math.round((now.getTime() - start_time[ctrl['mode']].getTime()) / 1000)
        ...
        document.getElementById('remaining_time_' + ctrl['mode']).innerText = remaining_text
    } else {
